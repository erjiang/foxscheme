...
export default function expand(expr: Expr, env?: Hash): Expr {
  if (env === undefined) {
    env = new Hash();
  }
  ...
  if (expr instanceof Symbol) {
    ...
    if (expr.first() instanceof Symbol) {
       ...
       */
      let rator = expand(expr.first(), env) as Symbol;
        ...
        if (bindings === nil)
          ...
          while (i--) {
            ...
            if (!(lhs instanceof Symbol))
              throw new Error("In let statement, cannot bind " + lhs, "Expand")

            // replace lhs with gensym, expand rhs
            var newlhs = new Gensym(lhs.name())
            var rhs = expand(bindarr[i].second(), env)
      ...
      if (expr.length() > 3) {
        ...
        body = new Pair(new Symbol("begin"),
          Util.map(function (expr) {
            return expand(expr, newenv)
          },
          (<Pair>expr.cdr()).cdr()))
      }
      // single body
      else if (expr.length() === 3) {
        body = expand(expr.third(), newenv)
      } else {
        throw new Error("Got lambda with fewer than 3 terms: " + expr);
      }
